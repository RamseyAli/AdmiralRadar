...
public abstract class MapBasedElement extends ShipPanel {
	...
	public MapBasedElement(GUIController cx) {
		...
		try {
			...
			shipIcon = bi.getScaledInstance( x , (int) ( ( bi.getHeight() * x ) / ( (float) bi.getWidth() ) ) ,
					Image.SCALE_DEFAULT );
	...
	public void draw() {
		...
		if (currentMouse.isValid()) g.fillOval( x0 - Y * R + (int) ( currentMouse.getX() * sp ) ,
				y0 - Y * R + (int) ( currentMouse.getY() * sp ) , 2 * Y * R , 2 * Y * R );
